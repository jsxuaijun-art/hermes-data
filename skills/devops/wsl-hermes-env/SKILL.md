...
**🔑 2026.8 新发现：Clash 只绑 127.0.0.1（Allow LAN 关）时，WSL 唯一通路 = `curl.exe`（Windows 原生）**

- 症状：`curl http://172.x.x.x:7890` 全部超时/拒绝；WSL 内 `127.0.0.1:7890` 回环也连不上（NAT 模式 localhost 转发不可靠）
- 根因：Clash 监听在 `127.0.0.1:7890`（Windows `netstat -ano | findstr :7890` 可见，仅 TCP 127.0.0.1），未开 Allow LAN → WSL 的 NAT 网段根本访问不到
- **解法**：调用 Windows 原生 curl.exe（跑在 Windows 网络栈里，能看到 127.0.0.1）：
```bash
# 连通性验证（返回 JSON = 通路 OK）
curl.exe -s --proxy "http://127.0.0.1:7890" 'https://api.github.com/repos/NousResearch/hermes-agent/releases/latest'
# 下载 tarball（大文件也稳，61MB 实测 OK）
curl.exe -L -s --proxy "http://127.0.0.1:7890" -o /mnt/e/hermes-upgrade/pkg.tar.gz 'https://github.com/.../archive/refs/tags/vTAG.tar.gz'
```
- 适用范围：GitHub API、raw.githubusercontent、release tarball —— 凡是 WSL 内连不上但 Windows 代理能上的，都用 curl.exe 代下，落 Windows 盘后 `cp` 回 /tmp
- ⚠️ 注意：curl.exe 是 Windows 程序，`-o` 路径必须是 Windows 可见路径（/mnt/e/... 等价 E:\...），不能直接写 WSL /tmp；下完再 cp

**版本号对应关系（2026.8 实测）**：官方 tag 是日期式（v2026.8.3），内部版本号在 pyproject.toml 的 `version = "0.20.0"`。用户说"升级到 v0.20"时，先列 tags 再核对 pyproject 确认目标 tag：
```bash
curl.exe -s --proxy "http://127.0.0.1:7890" 'https://api.github.com/repos/NousResearch/hermes-agent/tags?per_page=30'
curl.exe -s --proxy "http://127.0.0.1:7890" 'https://raw.githubusercontent.com/NousResearch/hermes-agent/vTAG/pyproject.toml' | grep -m1 '^version'
```
...
详见 `references/hermes-upgrade.md`。v0.15.1 → v0.20.0 完整实战（curl.exe 代理下载、混合目录 rsync 排除清单、launcher shebang 修复）见 `references/hermes-v0.20-upgrade.md`。
**Windows 侧同步升级**（C:\Users\Administrator\.hermes\hermes-agent，uv venv、cmd.exe UNC 陷阱、Start-Process 脚本落盘模式、GBK 编码）见 `references/hermes-windows-side-upgrade.md`。
...
## 🔴 Shell CWD 卡死陷阱 — 删除当前工作目录导致永久性终端不可用（2026.6.15 新增）

**场景**：在 WSL 终端中删除当前工作目录（如 `rm -rf ~/agent-sync` 且所在 session 的 cwd 是该目录），**所有后续 terminal 命令都会立即失败**，报错 `FileNotFoundError: [Errno 2] No such file or directory: '/原来的路径'`。

**根本原因**：每个 shell 会话都有一个内核级的 CWD（`/proc/<pid>/cwd`）。一旦该目录被删除：
- 内核将该路径标记为 `(deleted)`（通过 `ls -la /proc/<pid>/cwd` 可见）
- 所有 `cd`、`pwd`、任何文件名解析都以已删除的 CWD 为基准
- 即使执行 `cd /` 也失败——shell 内置的 `cd` 命令也卡在同一路径

**修复方法（无法修复，必须重建会话）**：

```bash
# 不能用 cd、pwd 或任何新命令——全部失败
# 唯一解法：关闭当前终端标签页，开一个新标签页

# 如果想在当前会话中恢复（以下命令也会失败，仅供参考）：
exec bash -c 'cd / && bash'  # ❌ 同一会话内任何命令都报错
```

**真正唯一有效的恢复方式**：
1. 在 PowerShell/CMD 中：`wsl -- shutdown` 然后重新 `wsl`
2. 或直接在 Windows 终端关掉标签页重新打开
3. 在 Hermes Agent CLI 中：会话已死，必须等 session 超时后新建 session

**预防**：
- 删除目录前先确认不在其中：`pwd` 检查当前路径
- 或者用安全删除模式：`cd /tmp && rm -rf /path/to/target`
- 注意，在 Hermes Agent 对话中，**terminal 工具的 cwd 会持久化**——如果上一个 terminal 调用恰好在即将删除的目录中，下一个调用会直接继承已删除的 cwd。写脚本时先 `cd /` 或 `cd ~` 保底。

**快速验证命令**（检查当前 cwd 是否正常）：
```bash
pwd && ls .
# 如果返回 FileNotFoundError → 会话已死，需要新建
# 如果正常返回当前路径 → 安全
```

**💡 最佳实践**：所有涉及 `rm -rf` 的 terminal 调用，在删除前先执行 `cd /`：
```bash
# ✓ 安全写法
cd / && rm -rf /mnt/c/Users/Admin/agent-sync

# ✗ 危险写法（如果当前在 agent-sync 中）
rm -rf /mnt/c/Users/Admin/agent-sync
```
...
- `references/hermes-v0.20-upgrade.md` — v0.15.1→v0.20.0 升级实录（2026.8.7）：curl.exe 走 Windows 代理下载、混合目录 rsync 排除清单、launcher shebang 修复、版本号对应关系
- `references/hermes-windows-side-upgrade.md` — Windows 侧 hermes 同步升级（2026.8.7）：uv venv（无 pip.exe）、cmd.exe UNC 陷阱、Start-Process 脚本落盘模式、GBK 编码、venv 损坏 STATUS_DLL_NOT_FOUND 重建
