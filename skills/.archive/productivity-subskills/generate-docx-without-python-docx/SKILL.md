...
7. **`w:pPr` inside tables**: Table cell paragraphs also need `w:pPr` for spacing — don't skip it.

8. **Python tuple cannot contain `key=value` — use dicts instead**: A recurring bug when structuring paragraph data for scripts:
   ```python
   # ❌ INVALID — tuple with keyword arguments
   ("赵总，...", "微软雅黑", "20", False, before=40, after=20, line=360),
   
   # ✅ CORRECT — dict literal
   {"text": "赵总，...", "font": "微软雅黑", "sz": "20", "bold": False, "before": 40, "after": 20, "line": 360},
   ```
   Then in the consuming loop, unpack with `**run` instead of `*run`:
   ```python
   for run in runs:
       script_parts.append(para(**run))  # ✅ — dict unpacking
   ```

9. **`import zipfile` must appear before its first usage in the function**: Python treats `zipfile` as local throughout a function if `import zipfile` appears anywhere in it. Accessing `zipfile.ZipFile(...)` before the import line raises `UnboundLocalError: cannot access local variable 'zipfile'`. **Fix**: Put ALL imports at the top of the function, before any usage. Or use global imports at module level.

10. **Misleading SyntaxError messages from `ast.parse()`**: A tuple with `key=value` inside it can produce a misleading error like `SyntaxError: 'return' outside function` pointing to a completely different line. The parser gets confused by the unexpected keyword syntax. **Debug technique**: If `ast.parse()` says error at line X but line X looks fine, suspect another syntax issue earlier in the file. Tokenize with `tokenize` module, binary-search line ranges, or extract suspicious sections (like inline data structures) and validate them in isolation.

> **Reference**: See `references/debug-syntax-error-workflow.md` for the full multi-pass debugging workflow used to resolve a real case.
