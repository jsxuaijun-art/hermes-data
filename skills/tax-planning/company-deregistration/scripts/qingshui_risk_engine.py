...
def _rule(key, keywords, risk, handle, redline, level="🔴", materials="", trigger_min=0.0, ratio_ref=None):
    RISK_RULES[key] = {
        "keywords": keywords, "risk": risk, "handle": handle,
        "redline": redline, "level": level, "materials": materials,
...
_rule("应收账款", ["应收账款", "应收帐款"],
      ...
      "无证据硬核销 → 税局不认，视同未分配利润", "🔴",
      materials="①清偿收回(银行回单)——无证据要求；②【核销路径才必需】若走坏账核销税前扣除，须备对方已注销/破产证据(公示系统截图+破产终结裁定书)+账龄超3年催收记录；若收回或正常纳入清算所得缴税，证据非必需")
...
_rule("存货", ["存货", "库存商品", "原材料", "在产品", "生产成本", "产成品"],
      ...
      "管理不善致存货损失 → 进项税额必须转出", "🔴",
      materials="变现=销售合同+发票+出库单；分配股东=股东会决议+视同销售计税；【申报损失税前扣除(不想纳税)】按2011年25号公告备证据：内部(盘点表+计税成本依据+责任认定)＋外部按原因——火灾=消防火灾认定/出警记录、暴雨=气象台天气证明+应急部门证明、交通事故=交警事故认定、投保=保险公司勘察理赔、被盗=公安报案/立案证明；正常损失(自然灾害/事故)可税前扣除，管理不善致损失→进项转出")
...
_rule("未分配利润", ["未分配利润"],
      ...
      "不能靠注销规避个税，税局按清算分配核定", "🔴",
      materials="股东会解散决议+清算分配方案+股息个税测算表(财税60号)")
...
_rule("股东借款", ["股东借款", "法人借款", "老板", "股东", "往来款"],
      ...
      "注销前结清或作分配处理",
      "12月底前未还的股东借款视同利润分配", "🟡",
      materials="归还=银行转账回单/收据；作分配=股东会决议+个税申报；垫付冲销=真实费用凭证+流水留证")
...
_rule("其他应收款", ["其他应收款"],
      ...
      materials="个人借款归还/作分配凭证(银行回单/股东会决议)；保证金押金应退=退款银行回单")
...
_rule("固定资产", ["固定资产"],
      ...
      materials="处置合同+发票+收款凭证；分配股东=股东会决议+视同销售计税；【申报损失税前扣除】按25号公告备证据：内部(计税基础资料+审批责任认定)＋外部——报废毁损=评估报告/专业机构检测报告+保险理赔、被盗=公安报案/立案/破案结案证明、火灾/水灾/交通事故=消防/气象/交警证明+保险勘察；⚠️即使提供也可能不被税局认可")
...
_rule("货币资金", ["货币资金", "银行存款", "库存现金", "现金", "其他货币资金"],
      ...
      materials="银行对账单/余额调节表；私户收款自查=对公流水+客户回款凭证，能解释来源才过关；【现金/银行存款损失申报税前扣除】现金盘点表+责任认定＋被盗=公安报案/立案证明、银行损失=银行证明+司法追索；⚠️现金丢失即使有报警记录，税局多数也不认可，从严别打包票")
...
_rule("应付账款", ["应付账款", "应付帐款", "其他应付款"],
      ...
      materials="【非必需】对方已注销/破产证据(公示系统截图+破产裁定+失联证明)仅当走'无法支付免缴'核销路径时才需备齐；【最简路径】直接转营业外收入缴25%企税，税款不流失，对方注销证据非必需——正常纳税即可。能清偿则留还款凭证")
...
_rule("预收账款", ["预收账款", "预收帐款", "合同负债"],
      ...
      "已履约的确认收入缴税；未履约的退款冲销",
      "长期挂预收不处理 → 税局疑隐匿收入", "🟡",
      materials="已履约=合同+履约交付+收入确认凭证；未履约退款=退款凭证/银行回单")
...
_rule("长期资产", ["长期待摊费用", "在建工程", "无形资产", "商誉", "长期股权投资", "长期应收款"],
      ...
      materials="处置合同+评估/作价文件；长投=被投资方注销/转让文件(子公司须先注销并附注销证明)")
...
_rule("应交税费", ["应交税费", "应交税金"],
      ...
      "贷方余额未清 → 无法取得清税证明", "🔴",
      materials="完税凭证/清缴证明；留抵退税申请材料；税控设备缴销凭证")
...
_rule("应付职工薪酬", ["应付职工薪酬", "应付工资", "应付福利费"],
      ...
      "注销前结清职工工资、社保",
      "欠薪注销 → 员工维权+清算组责任", "🟡",
      materials="工资发放表+银行代发回单+社保缴费凭证+个税申报记录")
...
def parse_subject_balance(path):
    ...
    if "借方" in "".join(subvals) or "贷方" in "".join(subvals):
        # 借/贷列从子行识别（表头行是合并表头）
        # 关键：表头可能是「期初/本期/本年/期末」多组借贷子行（如 R5: 借方|贷方 借方|贷方 借方|贷方 借方|贷方）
        # 雷区诊断要用【期末余额】列的借贷，即【最后一组】"借方/贷方"（覆盖式取尾），而非第一组（那是期初）。
        cols["debit_end"] = cols["credit_end"] = None
        for c in range(1, max_col + 1):
            sv = subvals[c-1]
            if sv == "借方":
                cols["debit_end"] = c      # 覆盖式：总是取最后一个"借方"
            elif sv == "贷方":
                cols["credit_end"] = c     # 覆盖式：总是取最后一个"贷方"
...
def gen_docx(findings, total_assets, out_path, tax=None):
    ...
    for f in findings:
        ...
        if f.get("materials"):
            P(f"材料/证据（清税前备齐）：{f['materials']}", 9, (0x2D,0x5F,0x8A))
        P("")
    H("四、注销计划与合法节税建议（遵守税法前提下减少税负）")
    plans = _gen_tax_plan(findings, tax)
    if not plans:
        P("暂无明显可节税项目；仍建议按法定流程办理。")
    for i, p in enumerate(plans, 1):
        H(f"计划{i}：{p['title']}", 12, (0x00,0x80,0x00))
        P(f"动作：{p['action']}")
        P(f"节税效果：{p['saved']}", c=(0x00,0x80,0x00))
        P("")
    H("五、总结性报告（老板版一句话结论 + 行动清单）")
    if findings:
        P(f"结论：该企业注销若按现状办理，预计注销合计涉税约 {abs(tax['qing_suo_tax']+tax['total_shareholder_tax']):,.0f} 元（清算所得税 {tax['qing_suo_tax']:,.0f} + 股东个税 {tax['total_shareholder_tax']:,.0f}）。", b=True)
    else:
        P("结论：未发现明显注销雷区，可按法定流程推进注销。", b=True)
    P("行动顺序（建议）:")
    steps = ["① 先做应收/存货/往来款的账务清理（注销前半年布局，证据链+账实一致）",
             "② 结清应交税费贷方余额，缴销发票及税控设备",
             "③ 无法清偿的应付按营业外收入处理并缴企税",
             "④ 股东/个人借款注销前归还或作分配，避免视同分红",
             "⑤ 申报清算所得缴纳清算所得税，剩余财产分配对股东补个税",
             "⑥ 取得《清税证明》→ 工商注销 → 社保注销 → 银行销户 → 印章缴销"]
    for s in steps:
        P("  " + s)
    P("")
    H("六、引流话术（获客用）")
    P("▸ 短视频/朋友圈版：", b=True)
    P("“公司不做了想注销？先别急着交资料。我见过太多老板，账上挂着应收账款收不回、存货一堆没处理、未分配利润几百万——等到了清税窗口，税局让你补的税比你想的多得多。注销不是交个执照，是一场清算体检。私信我，免费帮你看看账上埋了几颗雷。”")
    P("▸ 私信/群内版：", b=True)
    P("“老板，您公司要注销吗？我这边有套免费的‘注销清算体检’，把你家的科目余额表和资产负债表发我，我帮你看应收账款、存货、未分配利润、股东借款这几个最容易被税局盯上的科目，再给你算一笔注销到底要补多少税、怎么合法少缴。需要的话把报表发群里或私信我。”")
    P("▸ CTA：评论区留言“注销”或私信，领取免费清算体检。", c=(0xCC,0x00,0x00), b=True)
    P("")
    P("⚡ 本报告基于报表自动解析生成，节税建议均以遵守税法为前提，实际执行前请结合原始凭证及当地税局口径进一步核实。", 9, (0x99,0x99,0x99))
    doc.save(out_path)
    return out_path

def _gen_tax_plan(findings, tax):
    """根据诊断结果生成合法节税计划（遵守税法前提下减少税负）"""
    rules = {f['rule']: f for f in findings}
    plans = []
    # 应收账款 → 坏账核销税前扣除
    if "应收账款" in rules:
        v = abs(rules["应收账款"]["value"])
        plans.append({
            "title": "应收账款坏账核销，降低清算所得",
            "action": "对确实无法收回的应收款项，在注销前按2011年25号公告备齐证据（法院破产裁定/生效判决/账龄超3年+催收记录）作坏账损失税前扣除。",
            "saved": f"每核销{v:,.0f}元应收，相应减少清算所得，少缴企业所得税约{v*0.25:,.0f}元。",
        })
    # 存货 → 选择最优处置
    if "存货" in rules:
        v = abs(rules["存货"]["value"])
        plans.append({
            "title": "存货选择最优处置方式（变现/分配/报废）",
            "action": "区分正常损失（自然灾害等，可税前扣除）与非正常损失（管理不善，进项须转出）。正常损失作资产损失税前扣除，降低清算所得。",
            "saved": f"存货{v:,.0f}元若按正常损失处理可税前扣除，减少清算所得约{v*0.25:,.0f}元；避免违规进项转出。",
        })
    # 未分配利润 → 清算前合法分配/费用补提
    if "未分配利润" in rules:
        v = abs(rules["未分配利润"]["value"])
        plans.append({
            "title": "未分配利润，注销前合法降低个税基数",
            "action": "注销前用足可税前扣除项目（职工薪酬、社保、清算费用等），对股东借款先行归还，降低未分配利润基数。",
            "saved": f"未分配利润{v:,.0f}元，基数每降10万，股东股息个税少缴约2万元（20%）。",
        })
    # 股东/个人借款 → 归还避免视同分红
    if "股东借款" in rules or "其他应收款" in rules:
        _f = rules.get("其他应收款") or rules.get("股东借款") or {"value": 0}
        v = abs(_f["value"])
        plans.append({
            "title": "股东/个人借款，注销前归还避免视同分红",
            "action": "核查其他应收款中股东/关联方个人借款，注销前归还或作合法分配，避免被税局视同利润分配补20%个税。",
            "saved": f"其他应收款{v:,.0f}元中若含个人借款，归还后可避免{v*0.2:,.0f}元的视同分红个税。",
        })
    # 应付账款 → 暂估/无法清偿处理
    if "应付账款" in rules:
        v = abs(rules["应付账款"]["value"])
        plans.append({
            "title": "应付账款，区分暂估与无法清偿",
            "action": "对确实无需支付的应付转营业外收入（缴25%企税）前，先核实是否含暂估/虚挂可冲回；有依据的暂估核实后冲回，降低债务清偿收益。",
            "saved": f"应付{v:,.0f}元中可核减的暂估部分，能降低清算所得、减少企税。",
        })
    # 应交税费 → 结清欠税+用足留抵
    if "应交税费" in rules:
        v = abs(rules["应交税费"]["value"])
        plans.append({
            "title": "应交税费，结清欠税并争取留抵退税",
            "action": "注销前结清贷方欠税；若有借方留抵（进项大于销项）申请退税或结转，减少现金流出。",
            "saved": f"应交税费{v:,.0f}元，先结清欠税取得清税证明，留抵退税可回笼资金。",
        })
    return plans
...
def main():
    ...
    if findings or True:
        # 自动探测本机可写的真实桌面(跨机): 优先 D:\OneDrive\Desktop(本机), 再 C:\Users\<user>\Desktop
        out = None
        for cand in ["/mnt/d/OneDrive/Desktop", f"/mnt/c/Users/{os.environ.get('USER','Administrator')}/Desktop", "/tmp"]:
            if os.path.isdir(cand):
                out = os.path.join(cand, "注销雷区诊断报告.docx")
                break
        try:
