...
## 🚨 最高等级约束 — 表格格式红线（2026.5.12 更新 | 2026.5.26 强化）
...
### ⚠️ 2026.5.26 更新：不要"说"修正，要"做"修正

**用户明确表达：** 每次说了要改，但每次都改了再犯，犯了再改。**用户不需要我再保证或解释，只需要执行。**

**原则：** 如果发现我输出了 Markdown 表格或格式不对的表格，不要停下来解释为什么错了或我以后会改。直接重新生成正确格式输出即可。用户不需要听过程，只需要看结果。
