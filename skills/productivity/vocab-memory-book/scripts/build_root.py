# -*- coding: utf-8 -*-
"""
高考英语词汇默写本 · 词根版v3（最终用户确认版）
- Part 1: 词根词汇 50词一组（不同词根可混合），完整6轮艾宾浩斯 + 鼓励语
- Part 2: 基础词汇 50词一组，4轮复习（无学习/无终测）
- "#"列 0.2cm | 页码 2/365 | 音标两行 | 鼓励语多样性
"""
...
# ═══ 255+ Membean roots — ONE per line ═══
# ⚠️ CRITICAL: each root must be on its own line. 
# Do NOT put multiple roots on one line with | separator — parsing will break.
...
while i < len(root_words):
    ...
    root_groups.append({'label':'+'.join(roots_in[:3])+('...' if len(roots_in)>3 else ''), 'words':chunk})
    i += WPG

basic_groups = []
i = 0
while i < len(basic_words):
    basic_groups.append({'words':basic_words[i:i+WPG]})
    i += WPG
...
def set_bdr(c, **kw):
    ...
    for edge,val in kw.items():
        ...
        e.set(qn('w:space'),'0');b.append(e);p.append(b)
...
def build_cover(doc):
    ...
    tw=OxmlElement('w:tblW');tw.set(qn('w:w'),'11906');tw.set(qn('w:type'),'dxa');tblPr.append(tw)
    ...
    def ctr(b,a):
        ...
        p.paragraph_format.space_before=Pt(b);p.paragraph_format.space_after=Pt(a);p.paragraph_format.line_spacing=Pt(14)
        return p
    r=ctr(50,10).add_run('✨    ⭐    ✨    ✨    ⭐    ✨');r.font.size=Pt(18);r.font.color.rgb=RGBColor(0xFF,0xE7,0x82)
    p=ctr(60,10);r=p.add_run('高考英语词汇默写本');r.font.size=Pt(40);r.font.bold=True;r.font.color.rgb=RGBColor(0xFF,0xFF,0xFF)
    p=ctr(8,16);r=p.add_run('词根版 · 乱序编排 · 艾宾浩斯记忆法');r.font.size=Pt(16);r.font.color.rgb=RGBColor(0xFF,0xD7,0x66)
    p=ctr(40,12);r=p.add_run('🌊🌊  ⛵  ⛵⛵  🌊🌊🌊  ⛵  🌊🌊');r.font.size=Pt(24)
    p=ctr(36,12);r=p.add_run(f'词根词汇 {len(root_words)}词 ({len(root_groups)}组) · 基础词汇 {len(basic_words)}词 ({len(basic_groups)}组) · 共 {TOTAL_WORDS}词');r.font.size=Pt(11);r.font.color.rgb=RGBColor(0xBB,0xDD,0xFF);r.font.italic=True
    p=ctr(20,8);r=p.add_run('「今天的每一分努力」\r都是明天看得见的风景');r.font.size=Pt(12);r.font.color.rgb=RGBColor(0xDD,0xEE,0xFF);r.font.italic=True
...
# ═══ PAGE FOOTER (2/365) ═══
def add_footer(doc):
    section=doc.sections[-1];footer=section.footer;footer.is_linked_to_previous=False
...
def card_learn(doc, w):
    ...
    if w['root']: r=p.add_run(f"🔬 词根【{w['root']}】= {w['root_meaning']}");r.font.size=Pt(7.5);r.font.italic=True;r.font.color.rgb=RGBColor(0x2C,0x3E,0x50)
    else: r=p.add_run("📖 基础词汇");r.font.size=Pt(7.5);r.font.italic=True;r.font.color.rgb=RGBColor(0x2C,0x3E,0x50)
    ...
    for rowr in t.rows:
        for cell in rowr.cells: set_bdr(cell,top={'val':'single','sz':'4','color':'DDD'},bottom={'val':'single','sz':'4','color':'DDD'},
                start={'val':'single','sz':'4','color':'DDD'},end={'val':'single','sz':'4','color':'DDD'})
    doc.add_paragraph().paragraph_format.space_before=Pt(0);doc.add_paragraph().paragraph_format.space_after=Pt(0)
...
def sixcol(doc, wl, mode):
    ...
    if tblPr is None: tblPr=OxmlElement('w:tblPr');tbl.insert(0,tblPr)
    tw=OxmlElement('w:tblW');tw.set(qn('w:w'),'11000');tw.set(qn('w:type'),'dxa');tblPr.append(tw)
    ...
    for ri in range(rn):
        for col_offset, idx in [(0, ri),(3, ri+rn)]:
            ...
            v=OxmlElement('w:vAlign');v.set(qn('w:val'),'center');c._tc.get_or_add_tcPr().append(v)
            c=t.cell(ri+1,col_offset+1);c.text='';p=c.paragraphs[0];p.alignment=WD_ALIGN_PARAGRAPH.CENTER
            p.paragraph_format.space_before=Pt(0);p.paragraph_format.space_after=Pt(0)
            if mode=='en2cn': r=p.add_run(f"{sm}{w['word']}");r.font.size=Pt(10);r.font.bold=True;r.font.color.rgb=RGBColor(0x1A,0x1A,0x2E) if w['stars']>=4 else RGBColor(0x33,0x33,0x33)
                ...
                if idx%2==0: r=p.add_run(f"{sm}{w['word']}");r.font.size=Pt(10);r.font.bold=True;r.font.color.rgb=RGBColor(0x1A,0x1A,0x2E) if w['stars']>=4 else RGBColor(0x33,0x33,0x33)
            ...
            if mode=='en2cn': r=p.add_run(f"({'＿'*14})")
                ...
                # ⭐ Two lines: 英___ / 中___ (not on same line)
                r=p.add_run(f"英[{'＿'*8}]");r.font.size=Pt(9);r.font.color.rgb=RGBColor(0xCC,0xCC,0xCC);r.add_break()
            ...
            if mode!='phonetic': r.font.size=Pt(9);r.font.color.rgb=RGBColor(0xCC,0xCC,0xCC)
            v=OxmlElement('w:vAlign');v.set(qn('w:val'),'center');c._tc.get_or_add_tcPr().append(v)
            for cix in range(3):
...
# ═══ ENCOURAGEMENT ═══
# 20 milestones × 3-4 messages each, random.choice per trigger
ENCOURAGE = [
    (5,["🎉 你已完成总词汇量的5%，加油！好的开始是成功的一半！","🚀 5%完成！每一步都在靠近目标！","🌱 种子已经种下，5%的词汇正在生根发芽！"]),
    (10,["💪 很厉害！你又进一步，记住了总词汇量的10%！","📈 10%达成！你已经看到了进步的趋势！","🎯 十分之一完成，九分之九还在路上！"]),
    (15,["✨ 百尺竿头，更进一步，马上要完成15%了！","🔥 15%！记住的单词正在悄然改变你的大脑！","🏃 跑过了15%的路程，节奏已经起来了！"]),
    (20,["🚀 已经完成20%了！词根记忆法正在发力！","⭐ 五分之一的词汇已被征服！","💎 20%！累计的词汇量开始产生化学反应！"]),
    (25,["🌟 四分之一的词汇已经被你征服！","🎊 25%！四分之一里程碑，值得庆祝！","📚 一本词典的四分之一已经装进脑子里了！"]),
    (30,["🔥 30%！记住的每个词都会在考试中帮你得分！","⚡ 接近三分之一了！","🎪 30%达成！越到后面词根串记越轻松！"]),
    (35,["💎 超过三分之一了！你比昨天的自己更强大！","🌈 35%！越来越多的单词变得面熟了！","🏗 词汇大厦已经建了三成五！"]),
    (40,["⚡ 40%的词汇量，你已经超过大多数人了！","🎯 四成完成！节奏稳了！","💰 每记一个词都是在为未来存钱！40%了！"]),
    (45,["🎯 45%！距离半程只差一步之遥！","🏔 45%！登顶前的最后冲刺！","👀 45%完成，你已经开始用英语思维了！"]),
    (50,["🎊 哇，加油啊，已经过半了！50%的词汇已拿下！","🥇 半程冠军！","🏁 50%！一半已过，剩下的都是下坡路！","🎆 里程碑！所有单词你已认识一半！"]),
    (55,["🌈 55%！下半程开始，冲刺吧！","⛰ 55%！回头看，起点已经很远了！","📊 55%完成率，你的坚持令人敬佩！"]),
    (60,["🏆 60%！大多数词汇你都已经掌握了！","🎯 六成完成！量变正在引起质变！","🚀 60%！从这往后越来越顺！"]),
    (65,["⭐ 65%！三度已过二，胜利在望！","💪 接近七成！","🌄 65%！山顶越来越近了！"]),
    (70,["🚩 70%！你已经进入高阶词汇区了！","👑 七成！你的英语水平上了个大台阶！","🔥 70%！离精通只差30%！"]),
    (75,["🎪 四分之三！剩下的都是小菜一碟！","🏗 75%！词汇大厦即将封顶！","🎯 四分之三完成，你已经是词汇高手了！"]),
    (80,["👑 80%！你已经是一位词汇达人了！","💎 八成！剩下的20%用联想记忆轻松拿下！","🌟 80%！你已突破了英语词汇的关键门槛！"]),
    (85,["💥 85%！离胜利只差最后一程！","🏃 冲刺阶段！最后15%需要坚持！","🎆 85%！回头看一路走来，所有的努力都值得！"]),
    (90,["🎆 90%！最后的冲刺，冲鸭！","🏁 九成完成！最后10%决定胜负！","🥇 90%！你已站在词汇金字塔的顶端！"]),
    (95,["🏁 95%！胜利就在眼前！","🚀 最后5%，冲刺！","🌟 95%！你是千分之五十的坚持者！"]),
    (100,["🥇 🎉🎉🎉 100%！恭喜你完成了全部词汇的学习！","🎊🏆🎊 100%完美通关！","💎 100%！所有的汗水在这一刻绽放！"]),
]

def encourage(g_idx, total_g, this_words, words_before):
    cum = words_before + this_words
    pct = cum * 100 / TOTAL_WORDS
    best = None
    for mil, msgs in ENCOURAGE:
        if pct >= mil: best = msgs
...
for t in [f'📖 Part 1: 词根词汇 {len(root_words)}词/{len(root_groups)}组，完整6轮艾宾浩斯',
          f'📝 Part 2: 基础词汇 {len(basic_words)}词/{len(basic_groups)}组，4轮复习无学习',
          '🎲 各组内乱序编排 | ✍ 写作指数基于真实语料库频次 | 📄 页脚 2/365 页码']:
    p=doc.add_paragraph();p.paragraph_format.space_before=Pt(1);p.paragraph_format.space_after=Pt(1)
    r=p.add_run(t);r.font.size=Pt(8.5);r.font.color.rgb=RGBColor(0x44,0x44,0x44)

# ═══ PART 1: ROOT WORDS ═══
print(f"=== PART 1: {len(root_groups)} groups ===")
cumulative = 0
for gi, g in enumerate(root_groups):
    ...
    doc.add_page_break()
    p=doc.add_paragraph();p.alignment=WD_ALIGN_PARAGRAPH.CENTER
    r=p.add_run(f'DAY 1  学习页  · Part 1 · 第{gi+1}/{len(root_groups)}组');r.font.size=Pt(14);r.font.bold=True;r.font.color.rgb=RGBColor(0x1A,0x1A,0x2E)
    p=doc.add_paragraph();p.alignment=WD_ALIGN_PARAGRAPH.CENTER
    r=p.add_run(f'词根: {g["label"]}  ·  {tw}词汇');r.font.size=Pt(8);r.font.color.rgb=RGBColor(0x99,0x99,0x99)
    for w in wl: card_learn(doc,w)
    for tag,sub,mode in [('DAY 2','英→中','en2cn'),('DAY 4','中→英','cn2en'),('DAY 7','🔊 音标','phonetic')]:
        doc.add_page_break()
        review_title(doc,f'{tag} · Part 1 · 第{gi+1}组',f'{sub} · {g["label"]} · {tw}词');name_date(doc,tw);sixcol(doc,wl,mode)
    sl=[w for w in wl if w['stars']>=4]
    if sl:
        doc.add_page_break()
        review_title(doc,f'DAY 15 · Part 1 · 第{gi+1}组',f'★ 高频词 · ⭐{len(sl)}词');name_date(doc,len(sl));sixcol(doc,sl,'cn2en')
    doc.add_page_break()
    review_title(doc,f'DAY 30 · Part 1 · 第{gi+1}组',f'🎲 混合终测 · {g["label"]} · {tw}词');name_date(doc,tw);sixcol(doc,wl,'mixed')
    cumulative += tw
    msg = encourage(gi, len(root_groups), tw, cumulative - tw)
    if msg:
        ...
        r=p.add_run(msg);r.font.size=Pt(9);r.font.bold=True;r.font.color.rgb=RGBColor(0xC0,0x39,0x2B)
    print(f" {time.time()-dt_start:.0f}s")

# ═══ PART 2: BASIC WORDS ═══
print(f"\n=== PART 2: {len(basic_groups)} groups ===")
for gi, g in enumerate(basic_groups):
    ...
    for tag,sub,mode in [('DAY 2','英→中','en2cn'),('DAY 4','中→英','cn2en'),('DAY 7','🔊 音标','phonetic')]:
        doc.add_page_break()
        review_title(doc,f'{tag} · Part 2 · 第{gi+1}/{len(basic_groups)}组',f'{sub} · 基础词汇 · {tw}词');name_date(doc,tw);sixcol(doc,wl,mode)
    sl=[w for w in wl if w['stars']>=4]
    if sl:
        doc.add_page_break()
        review_title(doc,f'DAY 15 · Part 2 · 第{gi+1}/{len(basic_groups)}组',f'★ 高频词 · ⭐{len(sl)}词');name_date(doc,len(sl));sixcol(doc,sl,'cn2en')
    print(f" {time.time()-dt_start:.0f}s")
...
print(f"\n✅ {OUT} ({fsize:.0f}KB)")
print(f"📊 {TOTAL_WORDS}词 | Part 1: {len(root_words)}词/{len(root_groups)}组 | Part 2: {len(basic_words)}词/{len(basic_groups)}组")
print(f"🎯 {len(root_groups)}组×6轮 + {len(basic_groups)}组×4轮 | ✍ 词频覆盖: {len([w for w in all_words if w['word'] in freq_data])}/{TOTAL_WORDS}")
