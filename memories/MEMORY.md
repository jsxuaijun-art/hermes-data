...
§
2026.5.11 江姐表格竖线对齐终极修复：问题根因是U+2601（☁）等Ambiguous Width字符被U+FE0F（emoji呈现选择器）修饰后，现代终端渲染为2格，但原始wcwidth算1格+0格=1格，导致计算机padding比实际少1格、右侧边框偏左。修复方案：在display_width()中添加上下文检测——任何字符后面紧跟U+FE0F（VS16）且wcwidth原始值为1的，强制升级为width=2，并正确跳过后续U+FE0F和U+200D（ZWJ）序列。已验证通过：13行表格dw全部=44/50对齐。江姐确认修复成功并要求记住此方案。