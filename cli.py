...
def _detect_file_drop(user_input: str) -> "dict | None":
    ...
    # Early exit for slash commands: if input starts with "/" followed
    # by a single-word command (no internal "/" in the first token),
    # skip path resolution entirely.  Otherwise long command arguments
    # like `/goal <prose>` get passed to os.stat() and raise
    # ENAMETOOLONG on Linux (errno 36) or on WSL's /mnt/ fuse layer.
