...
## 只拉取/安装单个 skill（不动整个仓库）

> 用户指定「从 GitHub 拉取某个 skill 并重新加载」时，**不要**对镜像仓库做整库 `reset --hard`——
> 那会丢弃本地领先/独有的提交（本地 `ahead N`、`behind M` 时尤其危险）。正确做法是从 `origin`
> 定向抽出该 skill 目录，仅覆盖目标技能。步骤：

```bash
SYNC="<镜像仓库路径>"          # 见下方「定位镜像仓库路径」
TMP="/home/<user>/.hermes/cache/scratch/<short-name>"

# 0) fetch origin（只下载不合并）
git -C "$SYNC" fetch origin --prune

# 1) 先列远端 skills 树，确认目标 skill 的确切路径（远端目录结构可能已重组，勿假设与本地相同）
git -C "$SYNC" ls-tree -r --name-only origin/main skills/ | grep <skill名相关词>

# 2) archive 单目录到临时目录（不解引用 CRLF/不走工作区）
mkdir -p "$TMP"
git -C "$SYNC" archive origin/main skills/<category>/<skill-name> | tar -x -C "$TMP"

# 3) 落到 Hermes 实际加载技能的目录（~/.hermes/skills/，不是克隆目录）
cp -rf "$TMP/skills/<category>/<skill-name>" /home/<user>/.hermes/skills/<category>/

# 4) 验证：文件数 + 主 SKILL 行数 + skill_view 确认 readiness: available
```

归档目录结构若为 `skills/<category>/<skill-name>/`，复制时要重建到 `~/.hermes/skills/<category>/`，
并确认目标目录不存在旧版本（先删后拷，避免残留旧文件）。`git archive origin/main` 从提交对象直接
解出内容，不依赖工作区状态，是只读、无副作用的抽取方式。

### 定位镜像仓库路径（勿写死）

技能正文多处硬编码 `C:\Users\Admin\hermes-sync` + WSL 用户 `dmin`，但**这台机器不一定如此**——
实际 clone 可能位于 `C:\Users\<actual>\Documents\GitHub\hermes-data`，WSL 用户为 `jsxuaijun`。
动手前先探测真实路径（用 search_files / find + `ls`），不要假设 `hermes-sync` 目录存在：

```bash
find /mnt/c/Users -maxdepth 3 \( -name 'hermes-data' -o -name 'hermes-sync' \) -type d 2>/dev/null
whoami                 # 确认当前 WSL 用户
```

Windows git.exe 路径含空格：`/mnt/c/Program Files/Git/bin/git.exe` 在 bash 里**必须**加引号或
走 `git`（WSL 自带 git 通常已够用）；裸写 `Program Files` 会被按空格拆成多条命令。
...
### 17. 🟡 整库 `reset --hard` 在 /mnt/c 桥接上会超时被杀 — 用后台进程跑长 wait

**适用**: 用户明确要求"整个镜像仓库对齐远端、可以覆盖本地"（非单 skill 抽取）时，`reset --hard`
在 WSL↔Windows 文件桥接上要重写数千文件，耗时会超过前台终端超时（本机实测 ~63s 就被杀，
exit -1、工作树未变），反复重试仍被中断。

**修复**: 换成后台运行 + 长 `wait`，让它跑完再验证：

```bash
# 后台 + notify，随后 process_manage action=wait timeout=300 轮询到 RESET_OK
terminal(command='cd "<SYNC>" && git reset --hard <commit> && echo "RESET_OK $(git rev-parse --short HEAD)"', background=true, notify=true)
```

**关键规则**:
- 覆盖前先 `git branch -f backup-<tag> HEAD` 建备份分支，确认指向本地 HEAD，覆盖后仍可找回独有提交。
- `wait` 会被工具钳到其上限（如 300→180s），大仓库可能不够；必要时 `poll` 续等。
- 完成标准不是命令返回 0：要 `git status -sb` 无 ahead/behind、`git rev-parse --short HEAD` == origin。
