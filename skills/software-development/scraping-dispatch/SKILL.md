...
## 抓取→分析→创作 双模型流水线（2026.8.10 太仓金店偷税案定型）
完整链路：**抓公告 → deepseek-v4-flash 分析 → kimi-k3 创作**，产出公众号文章+短视频脚本。
- 调用中转站 chudian API（OpenAI兼容），API key 从本机 hermes 环境变量读（.bashrc 可查，勿在脚本里硬编码）。
- 写脚本文件再跑（勿用 python -c heredoc，转义易错）；脚本模板见 `/tmp/llm_pipeline.py`（分析/创作双模式）。
- ⚠️ **kimi-k3 渠道可能 503 不可用**（服务商端暂无渠道，非配置问题）→ 降级用 deepseek-v4-flash 出稿，质量同样可用，稍后可重试 kimi 对比。
- 公众号文章发布走 wechat-publish skill；短视频脚本存桌面 C:\Users\Admin\Desktop\（交付物一律 C 盘桌面）。
